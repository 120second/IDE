...
<section class="visual-preview">
  <header class="generator-step-header">
    <div class="generator-step-copy"><strong>数据预览</strong>{#if selected}<span>种子 {selected.seed}</span>{/if}</div>
  ...
  {:else}
    <div class="visual-preview-empty"><strong>在这里查看生成的数据</strong><span>设置输入格式，再点击“生成预览”。</span></div>
  {/if}
</section>
