...
| 模板中心 | 本地/云端独立模板库、代码片段与文件模板、嵌套分类、拖放整理、收藏、搜索、版本历史、模板讲义预览及紧凑双栏 PDF/打印 |
...
- SQLite 保存模板、测试点、归档、生成规则和最近工作区等本地数据，断网和未登录时可正常使用。
- `lightcp-server` 使用 FastAPI、SQLAlchemy 和独立 SQLite 提供可选的账号及模板云端 API；登录入口位于设置，不会阻塞 IDE 启动。
...
桌面端默认通过 `https://114.55.109.162/lightcp-api` 访问云端 API。
如需连接本机 API，可在启动前临时设置
`LIGHTCP_API_BASE_URL=http://127.0.0.1:8100/api`；也可以通过该环境变量切换到其他 HTTPS 部署。`npm run dev` 只会启动
Vite 网页服务，浏览器里无法调用 Tauri 提供的 Rust 后端。发布版允许通过
HTTP 连接回环地址；连接远程服务时仍必须使用 HTTPS。
...
模板中心右上角的“打印/导出”可以把代码片段和文件模板整理成 A4 讲义。预览按真实书页分页：第 1 页单独置于右侧，后续页面以书本双页展开；可用方向键翻页、按住空白区域拖动画布，并通过缩放滑杆、加减按钮或“适应”按钮调整预览大小。讲义会按 Chapter、Section 和模板生成分级目录，并附带轻量页眉、页脚与连续页码。正文严格按“本页左栏 → 本页右栏 → 下一页”顺序排版，不使用瀑布流回填：短代码优先整块换栏，长代码才会续栏并优先在函数、空行或右花括号附近分割，明显超宽的代码会临时横跨双栏，随后恢复顺序双栏。允许保留少量有助阅读的空白，不会为了填满页面打乱模板顺序。全部页面参数集中在 `PrintLayoutConfig`，新增模板会自动套用相同规则。屏幕预览保持 LightCP 暗色语法高亮，打印和 PDF 自动切换为白底、灰度层级的省墨主题；还可切换统一单栏或极致省纸模式，并选择是否显示说明、元数据和代码行号。
...
工作区源文件始终留在用户选择的目录中。测试点、归档元数据、模板历史、随机生成规则和本地模板均保存在本机，无需登录。云端模板是独立的可选存储区；联网并登录后，可以将本地模板上传到云端，也可以把云端缺少的内容同步到本地。两种同步都只新增缺失项，不删除或覆盖已有模板。JWT 只保存在操作系统凭据库，不写入 `localStorage` 或 SQLite。
